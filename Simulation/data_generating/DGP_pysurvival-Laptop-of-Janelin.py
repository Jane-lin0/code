...
class SimulationModel():
    ...
    * risk_parameter: double (default = 1.)
    ...
    def random_data(N, index):
        ...
        # index = np.random.binomial(n = 4, p = 0.5)  # 为什么 n = 4
        distributions = {
            'uniform': np.random.uniform(low=0, high=1.0, size=N),
            # 'uniform_b': np.random.uniform(low=-20.0, high=100.0, size=N ),
            'binomial_a': np.random.binomial(n=4, p=0.6, size=N),
            # 'binomial_b': np.random.binomial(n = 200, p = 0.6, size = N ),
            'exponential_a': np.random.exponential(scale=2, size=N),
            # 'exponential_b': np.random.exponential(scale=0.01, size = N ),
            'normal_a': np.random.normal(loc=5.0, scale=3.0, size=N),
    ...
    def hazard_function(self, t, BX):
        ...
        if self.survival_distribution.lower().startswith('exp'):
            ...
            return _lambda * self.nu * np.power(t, self.nu - 1)

            # Gompertz
        elif self.survival_distribution.lower().startswith('gom'):
            return _lambda * np.exp(self.nu * t)

        # Log-Logistic
        elif self.survival_distribution.lower().endswith('logistic'):
            numerator = _lambda * self.nu * np.power((_lambda * t), self.nu - 1)
            denominator = (1 + np.power((_lambda * t), self.nu))
            return numerator / denominator

        # Log-Normal
        elif self.survival_distribution.lower().endswith('normal'):
            arg_normal = (np.log(t) - np.log(_lambda)) / self.nu
            numerator = (1. / (t * self.nu)) * scipy.stats.norm.pdf(arg_normal)
    ...
    def survival_function(self, t, BX):
        ...
        # Weibull
            ...
            return np.exp(-np.power(t, self.nu) * _lambda)

        # Gompertz
        elif self.survival_distribution.lower().startswith('gom'):
            return np.exp(-_lambda / self.nu * (np.exp(self.nu * t) - 1))

        # Log-Logistic
        elif self.survival_distribution.lower().endswith('logistic'):
            return 1. / (1. + np.power(_lambda * t, self.nu))

        # Log-Normal
        elif self.survival_distribution.lower().endswith('normal'):
            arg_cdf = (np.log(t) - np.log(_lambda)) / self.nu
    ...
    def generate_data(self, num_samples=100, num_features=3,
        ...
        * dataset: pandas.DataFrame
        ...
        if feature_weights is None:
            self.feature_weights = [1.] * self.num_variables  # 系数全是1
        ...
        X = np.zeros((num_samples, self.num_variables-1))
        columns = []
        for i in range(self.num_variables-1):
            # key, X[:, i] = self.random_data(num_samples)
            # columns.append( 'x_' + str(i+1) )
            key, X[:, i] = self.random_data(num_samples, i)
            columns.append('x_' + str(key))  # 输出对应分布
        ...
        C = np.random.uniform(low=0, high=treatment_col).flatten()         # 删失时间服从均匀分布
        # C = np.maximum(C, 0)
