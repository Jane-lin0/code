...
class CounterfactualSurvFtn():
    ...
    def data_generate(self, sample_num, survival_distribution, test_size):
        """
        @param sample_num:
        @param survival_distribution: Weibull, Gompertz, Log-Logistic, Log-Normal
        @param test_size:
        @return:
        """
        ...
        df_train = train_test_data_split(dataset, test_size=test_size, save_path=self.data_path)
    ...
    def fit(self, bandwidth_list):
        ...
        for h in bandwidth_list:
            ...
            for i in range(self.cv):
                ...
                error = self.estimate_error_imse(survival_pred,
                                                 treatment_testSet=df_validation['a'],
                                                 lambda_testSet=df_validation['lambda'])
    ...
    def predict(self, df_train, df_validation, cde_estimates, a_grid):
        """
        @param df_train:
        @param df_validation:
        @param h: bandwidth
    ...
    def estimate_error_imse(self, survival_pred, treatment_testSet, lambda_testSet):
        # IMSE 评估生存函数估计
        survival_true_values = survival_true(self.treatment_grid, self.time_grid,
    ...
    def estimate_error_mse(self, survival_pred, treatment_testSet, lambda_testSet):
        ...
        survival_true_a = survival_true(self.treatment_grid[idx], self.time_grid,
                                        treatment_testSet=treatment_testSet, lambda_testSet=lambda_testSet)
...
#     weight = np.vstack([weight, w_normalization.reshape(1, -1)])
#     # final result: weight = { ndarray:(len(treatment_grid), len(df_test))}
# counterfactual_survival = weight @ conditional_survival_estimated  # 没有分清 train validation test
