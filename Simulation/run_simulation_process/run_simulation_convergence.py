...
test_size = 0.15
bandwidth_list = np.logspace(-2, 0, num=10)  # 0.01 至 1 之间的 10 个数

model = CounterfactualSurvFtn(path=path, cv=cv)

# 数据生成
model.data_generate(sample_num=N, survival_distribution='exponential', test_size=test_size)
...
model.fit(bandwidth_list=bandwidth_list)
...
# 误差评估
imse = model.estimate_error_imse(counterfactual_survival_pred, df_test['a'], df_test['lambda'])
mse = model.estimate_error_mse(counterfactual_survival_pred, df_test['a'], df_test['lambda'])
# 补充 RMSE、Δq
