...
'''
# cv = 5
path = r"C:\Users\janline\OneDrive - stu.xmu.edu.cn\桌面\empirical_data\ANdata\AN"
...
mse_for_h = []
imse_for_h = []
summary_median_survival_pred = np.empty(shape=(0, 90))
# summary_median_survival_true = np.empty(shape=(0, 200))
h_list = [1, 0.75, 0.5, 0.25]
for h in h_list:
    i = 0
    df_train = pd.read_excel(path + f"data_{i}.xlsx", sheet_name='train')
    df_test = pd.read_excel(path + f"data_{i}.xlsx", sheet_name='test')
    ...
    # mse = mean_squared_error_normalization(survival_estimate_a, survival_true_a, time_grid)  # 无法知道真实数据的生存函数
    ...
    summary_survival_output = np.hstack((treatment_grid[row_index], counterfactual_survival_output))
    ...
    for idx, color in zip(treatment_idx, colors):
        ...
        j += 1
    plt.legend()
    plt.xlabel('pass_rate_change')
    plt.ylabel('probability')
    # 将图像保存到本地
    # filename = f"h{int(100 * h)}.png"
    filename = f"ANdata_h{int(100 * h)}.png"
    filepath = os.path.join(r"C:\Users\janline\OneDrive - stu.xmu.edu.cn\桌面\empirical_data\ANdata", filename)
...
summary_median_survival_output = np.vstack((treatment_grid.T, summary_median_survival_pred)).T[row_index]
...
for ndarray in [summary_median_survival_output]:
    print_latex(ndarray)
